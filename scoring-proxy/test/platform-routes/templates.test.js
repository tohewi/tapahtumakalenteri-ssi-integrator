...
describe('GET /api/v1/platform/tenants/:tenantId/templates', () => {
  it('returns 401 without authentication', async () => {
    const res = await request('GET', '/api/v1/platform/tenants/ten_123/templates');
    expect(res.status).toBe(401);
  });

  it('returns 200 with templates list', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner10@test.com' });
    const tenantId = regRes.data.tenant.id;
    const res = await request('GET', `/api/v1/platform/tenants/${tenantId}/templates`, { cookies: cookie });
    expect(res.status).toBe(200);
    expect(Array.isArray(res.data.templates)).toBe(true);
  });
});

describe('POST /api/v1/platform/tenants/:tenantId/templates', () => {
  it('returns 400 for missing name', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner11@test.com' });
    const tenantId = regRes.data.tenant.id;
    const res = await request('POST', `/api/v1/platform/tenants/${tenantId}/templates`, {
      cookies: cookie,
      body: {}
    });
  ...
  it('returns 400 for non-existent discipline', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner12@test.com' });
    const tenantId = regRes.data.tenant.id;
    const res = await request('POST', `/api/v1/platform/tenants/${tenantId}/templates`, {
      cookies: cookie,
      body: { name: 'Test Template', disciplineId: 'disc_nonexistent' }
    });
    expect(res.status).toBe(400);
  });

  it('returns 201 with created template', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner13@test.com' });
    const tenantId = regRes.data.tenant.id;

    const createDisc = await request('POST', `/api/v1/platform/tenants/${tenantId}/disciplines`, {
      cookies: cookie, body: { name: 'Test Disc' }
    });
    const discId = createDisc.data.discipline.id;

    const res = await request('POST', `/api/v1/platform/tenants/${tenantId}/templates`, {
      cookies: cookie,
      body: { name: 'Test Template', disciplineId: discId, ssiSeedEventId: 12345 }
    });
    
    expect(res.status).toBe(201);
    expect(res.data.template.name).toBe('Test Template');
    expect(res.data.template.disciplineId).toBe(discId);
  });
});

describe('GET /api/v1/platform/tenants/:tenantId/templates/:id', () => {
  it('returns 404 for non-existent template', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner14@test.com' });
    const tenantId = regRes.data.tenant.id;
    const res = await request('GET', `/api/v1/platform/tenants/${tenantId}/templates/tmpl_999`, { cookies: cookie });
    expect(res.status).toBe(404);
  });

  it('returns 200 with template', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner15@test.com' });
    const tenantId = regRes.data.tenant.id;

    const createDisc = await request('POST', `/api/v1/platform/tenants/${tenantId}/disciplines`, {
      cookies: cookie, body: { name: 'Test Disc' }
    });
    const createTempl = await request('POST', `/api/v1/platform/tenants/${tenantId}/templates`, {
      cookies: cookie, body: { name: 'Test Template', disciplineId: createDisc.data.discipline.id }
    });
    const tmplId = createTempl.data.template.id;

    const res = await request('GET', `/api/v1/platform/tenants/${tenantId}/templates/${tmplId}`, { cookies: cookie });
    expect(res.status).toBe(200);
    expect(res.data.template.name).toBe('Test Template');
  });
});

describe('PATCH /api/v1/platform/tenants/:tenantId/templates/:id', () => {
  it('returns 200 and updates template', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner16@test.com' });
    const tenantId = regRes.data.tenant.id;
    ...
    const res = await request('PATCH', `/api/v1/platform/tenants/${tenantId}/templates/${tmplId}`, {
      cookies: cookie, body: { name: 'Updated Template', ssiSeedEventId: 54321 }
    });
    
    expect(res.status).toBe(200);
    expect(res.data.template.name).toBe('Updated Template');
    expect(res.data.template.ssiSeedEventId).toBe(54321);
  });
});

describe('DELETE /api/v1/platform/tenants/:tenantId/templates/:id', () => {
  it('returns 200 and deletes template', async () => {
    const { cookie, res: regRes } = await registerAndGetCookie({ email: 'owner17@test.com' });
    const tenantId = regRes.data.tenant.id;
    ...
    const res = await request('DELETE', `/api/v1/platform/tenants/${tenantId}/templates/${tmplId}`, { cookies: cookie });
    expect(res.status).toBe(200);
    
    const getRes = await request('GET', `/api/v1/platform/tenants/${tenantId}/templates/${tmplId}`, { cookies: cookie });
