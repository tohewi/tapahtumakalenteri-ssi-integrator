...
function createMockDb({ existingRows = [], insertedRow = null, updatedRow = null, count = 0, listRows = [] } = {}) {
  const calls = []
  const client = {
    query: vi.fn(async (sql, params = []) => {
      ...
      if (sql.includes('ORDER BY selected_squad_number')) {
...
describe('registration-store', () => {
  ...
  it('inserts a new local registration before SSI sync', async () => {
    ...
    expect(result.registration).toMatchObject({ id: 'reg-1', email: 'matti@example.com', syncStatus: 'pending' })
  ...
  it('updates an existing active registration instead of creating a duplicate', async () => {
    const existing = rowFromParams([
      'reg-1', '150', 'Old Cup', null, 1, 'Laina-ase', 'Matti', 'matti@example.com', null, 'yes', 'matti@example.com', 'confirmed', 'synced', null, null,
    ])
  ...
  it('lists registrations for organizer views', async () => {
    const db = createMockDb({ listRows: [rowFromParams(['reg-1', '150', 'Cup', null, 1, 'Laina-ase', 'Matti', 'matti@example.com', null, 'no', null, 'confirmed', 'manual_needed', null, null])] })
  ...
  it('updates registration and sync status', async () => {
    const row = rowFromParams(['reg-1', '150', 'Cup', null, 1, 'Laina-ase', 'Matti', 'matti@example.com', null, 'yes', 'matti@example.com', 'confirmed', 'failed', 'SSI_DOWN', 'SSI unavailable'])
  ...
  it('records SSI sync attempts without exposing details to public paths', async () => {
    const db = createMockDb()
