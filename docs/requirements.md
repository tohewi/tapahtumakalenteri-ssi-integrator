...
| 46 | Single Authentication: Authentication to SSI and WordPress should happen only once at the beginning of any operation (single event or batch). Session tokens should be retained and reused throughout the entire process. No repeated OTP prompts during batch processing. | [x] Implemented - sessions passed via PreAuth parameter set |
