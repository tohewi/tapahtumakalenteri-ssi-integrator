...
| OTP-1 | **Gmail OTP Fetching for Tapahtumakalenteri Integration**: The tapahtumakalenteri (WordPress calendar) integration requires an OTP code sent via email. The system must be able to fetch the OTP from a Gmail account programmatically. **Inputs:** Gmail address, app password (Gmail IMAP), sender address filter, subject line filter. **Behaviour:** Connect to Gmail via IMAP, search for the most recent email matching the sender + subject filters, extract the OTP code from the email body, return it to the caller. **Security constraints:** (a) Must use Gmail App Password (not account password) since 2FA is required; (b) Must NOT read full mailbox contents — only fetch emails matching the exact sender + subject filter; (c) Credentials (mailbox, app password) provided per-integration, not hardcoded; (d) Minimize exposure: connect, fetch matching email, extract OTP, disconnect — no persistent mailbox connection. **Scope notes:** This unblocks R41 (PowerShell secrets management, currently parked). The OTP is needed during WordPress/tapahtumakalenteri authentication. Consider implementing as a standalone utility (`lib/email/gmail-otp.js`) usable from both Node.js event creation flows and potentially PowerShell scripts via a CLI wrapper. | ⬚ Backlog | Medium | Unblocks R41. Gmail App Password required. IMAP with narrow sender+subject filter. |
