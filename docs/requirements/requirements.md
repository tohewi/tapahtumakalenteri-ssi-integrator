...
| CAL-5 | **Calendar Statistics Update**: After event completion, query SSI for participant count and calculate shots fired (participants × 100), then update the corresponding WordPress calendar event. **Trigger:** Manual "Update stats" action in SchedulePage, or future cron/webhook after SSI event completion. **SSI query:** Scrape `/event/{ct}/{id}/participants/` for approved participant count (count `<abbr title="Approved">A</abbr>` matches). **WP update:** Use calendar adapter `updateEvent()` with ACF statistics fields (shots fired, attendee count, event count). Replaces `Update-TapahtumakalenteriEvent.ps1` (246 lines). | ⬚ Pending | Medium | Post-event operation. Can be manual initially, automated later |
| CAL-6 | **Calendar Data Integrity**: Cross-reference validation between SSI events and WordPress calendar events. **Checks:** (1) Every SSI Cup owned by the tenant's SSI account has a corresponding calendar event, (2) Calendar event permalink contains Cup ID, (3) Calendar event content links to SSI Cup URL, (4) Date list validation — all scheduled dates have both SSI and calendar entries. **Implementation:** `lib/calendar/integrity-check.js`. Exposed as API endpoint + UI in SchedulePage (admin action). Replaces `Test-EventIntegrity.ps1` (449 lines). Subsumes R47 (Data Integrity Check). | ⬚ Pending | Low | Operational tool. Subsumes R47 from Release 3.1 |
| MP3 | **Match Personnel Management — Gap Analysis**: The original MP3 envisioned extending the SRA Training staffing MVP into a general-purpose match personnel system. Release 9.0 (Event Staffing) implemented significant staffing capabilities: staffing needs, signups/withdrawals, leaderboard, role-based assignments, and email notifications. **This requirement is a gap analysis**: (1) Compare R9.0 implementation against MP3's original scope — identify what is covered and what is missing, (2) Evaluate per-discipline role definitions (currently only SRA-style roles exist), (3) Assess personnel availability management (not yet implemented), (4) Review personnel assignment visibility in match event context (partially done via SchedulePage staffing indicators), (5) Document gaps as new backlog requirements or mark MP3 as fully subsumed by R9.0. **Deliverable:** Gap analysis document or updated backlog requirements. | ⬚ Pending | Medium | R9.0 covers most of original MP3 scope. Gap analysis to confirm completeness or identify remaining work |
| MP8 | **Localization & Regional Settings**: Tenant-level localization configuration — city, country, timezone, locale. These settings propagate to event creation (SSI timezone, region fields), calendar publishing (event location), and UI display. Must handle: (a) default timezone for date/time display and schedule calculations, (b) country/region for SSI event region field, (c) city/venue for event venue and calendar location, (d) locale for date/number formatting (fi-FI, en-US, etc.). Stored on tenant record, overridable per template. **Calendar relevance:** CAL-3 needs location data for WordPress event ACF fields — currently hardcoded in PowerShell config, should come from tenant/template localization settings. | ⬚ Design | Medium | Feeds into CAL-3 (calendar location), CAL-4 (event execution). Originally R8.1 |
| MP9 | **Tenant Branding & Picture**: Allow tenants to upload a logo/picture for their organization. Displayed in tenant dashboard, member views, and optionally in calendar event content. Requires: (a) image upload endpoint with size/format validation (max 2MB, jpg/png/webp), (b) image storage (Render disk or S3-compatible), (c) serving via CDN-friendly URL, (d) UI for upload/preview/remove in tenant settings. | ⬚ Design | Low | Nice-to-have for multi-tenant UX. Originally R8.1 |
...
| TEN-1 | **Tenant Context & URL Strategy — Design + A/B Prototype**: The current model (login → select tenant → access that tenant's data) has UX friction and unclear scope when a user belongs to multiple tenants. Requires design work and likely a small A/B prototype before commitment. **Open questions**: (1) Should each tenant have its own URL or subdomain? (e.g., `app.towi.me/t/turres` or `turres.towi.me`) — pros: bookmarkable, deep-linkable, sharable; cons: complicates auth cookies, CORS, cert management; (2) Is login shared (one identity, all tenants) or tenant-scoped (login separately per tenant)? — current system is shared identity/common login; (3) After login, does the user enter a "home" showing all their tenants + cross-tenant event feed, or do they login into one tenant and switch via a dropdown? (4) Should a user with roles in 3 tenants see a unified "upcoming events" view aggregated across all three? **Design options to evaluate**: (A) **Global login + tenant switcher** (current approach, improved): common login, `/platform/` dashboard lists all tenants, switching sets active tenant context in session; clean URLs stay at same domain; (B) **Global login + tenant-scoped URLs** (`/t/:tenantSlug/`): after login, URLs are tenant-scoped; bookmarkable; browser back/forward works cleanly; tenant slug resolves without extra roundtrip; (C) **Tenant subdomain** (`turres.towi.me`): strongest isolation; requires wildcard cert + DNS; session cookies per subdomain (cookie sharing needs `domain=.towi.me`); complicates local dev. **Cross-tenant feed**: regardless of URL scheme, consider a top-level "My Events" view that shows all upcoming events the logged-in user has a role in, across tenants — useful for members who staff events in multiple clubs. **Recommendation direction**: start with option (B) — tenant-scoped path URLs (`/t/:slug/`) — as it balances discoverability, bookmarkability, and cookie simplicity. Prototype both (A) and (B) and gather feedback before full implementation. **A/B test plan**: implement (B) as a feature flag on top of (A); measure: time-to-first-action after login, support requests about "wrong tenant", navigation errors. | ⬚ Backlog | Medium | Design + prototype before implementation. Cross-tenant "My Events" feed is an independent sub-feature. |
| PEW-1 | **Post-Event Workflows — Framework**: It must be possible to configure workflows that execute after an event completes. Workflows are defined per template and triggered manually or automatically when an event reaches a terminal state. Each workflow has a type, configuration, and execution log. Framework should support sequential execution of multiple workflows per event. | ⬚ Design | Medium | Foundation for PEW-2, PEW-3, PEW-4 |
| PEW-2 | **Post-Event Workflow — Email Shooter Count Report**: Report shooter count (excluding DNS/DQ) via email to specified addresses (to, cc fields configurable per template). Email includes event name, date, and final shooter count. Uses existing Resend email infrastructure. | ⬚ Design | Medium | Depends on PEW-1 framework |
| PEW-3 | **Post-Event Workflow — Tapahtumakalenteri Statistics Update**: Report shooter count (excluding DNS/DQ) and shots fired (stage shot count × shooter count) via HTTPS to Tapahtumakalenteri (WordPress). Uses existing `WpCalendarAdapter.updateEvent()` to update ACF fields on the published calendar event. | ⬚ Design | Medium | Depends on PEW-1 framework + CAL-5 |
| PEW-4 | **Post-Event Workflow — Publish Scores (Mark Event Completed)**: Publish final scores by marking SSI events as completed. This makes results publicly visible on SSI. Requires confirmation step to prevent accidental publication. | ⬚ Design | Medium | Depends on PEW-1 framework |
| INT-1 | **Multi-System Integration Architecture — Design**: Tenants must be able to define which external systems Match Manager integrates with. Two integration categories exist today: **(1) Event Management & Scoring** — currently ShootNScoreIt (SSI); responsible for disciplines, event creation, squad management, scoring, and results. **(2) Event Scheduling & Reporting** — currently Tapahtumakalenteri by Reserviläisliitto (WordPress/ACF); responsible for calendar publishing, public event listings, and attendance statistics; uses Gmail for MFA/OTP during WordPress authentication. This selection has deep impact on all Match Manager functionality: disciplines, templates, event builders, scoring flows, and calendar publishing are all system-specific. **Design must address:** (a) tenant-level integration configuration (which systems are active, credentials per system); (b) abstraction layer so core business logic (templates, scheduling, workflows) is system-agnostic; (c) system-specific adapters behind a common interface (current `ssi-core/` and `wp-adapter` are a starting point but tightly coupled); (d) discipline and template binding to a specific event management system; (e) support for multiple systems per category (e.g., a tenant using both SSI and another scoring platform, or multiple calendar systems); (f) graceful degradation when an optional integration is not configured (e.g., no calendar system → skip calendar publishing). **Requires thorough detailed design** before implementation — affects data model, API surface, UI flows, and all existing integration code. | ⬚ Design | High | Architectural — blocks future system integrations. Current code is SSI+WP hardcoded. |
| PRF-1 | **Page-Load Latency: Pre-fetch & Cache Events + Staffing Data**: SchedulePage and RosterView are slow on first load because all data (events, staffing needs, templates) is fetched synchronously on mount. Investigate and implement a caching strategy that reduces perceived latency without excessive server load. **Constraints**: (a) at login time the tenant is unknown — user selects tenant after login, so tenant-scoped data cannot be pre-fetched at login; (b) multiple tenants per user is possible; (c) SSI event state can change externally (events executed, cancelled). **Options to evaluate**: (1) **Tenant-selection-triggered prefetch** — when user selects/switches a tenant, immediately begin fetching events, templates, and staffing data in parallel before navigation completes; (2) **Stale-while-revalidate (SWR)** — cache last-seen API responses in sessionStorage/memory, serve stale data instantly on mount, then silently refresh in background and re-render on change (fastest perceived load, risk: brief stale display); (3) **React Query or SWR library** — replace manual `useEffect` fetch patterns with a proper cache layer (deduplicated requests, background refresh, retry); (4) **Server-side ETags / 304 Not Modified** — backend returns `ETag` + `Last-Modified`; client sends `If-None-Match`; if unchanged, returns 304 with no body (reduces bandwidth, not initial latency); (5) **Lazy pagination** — load only the next 3 months of events on first render, load older/future on scroll. **Recommendation direction**: combine (1) + (2): prefetch on tenant select into a shared React context cache; use SWR pattern for staleness tolerance. Avoid (3) unless adding a dependency is approved. **Balance note**: prefetch only on explicit tenant selection (not speculatively for all tenants) to keep server load proportional to user intent. | ⬚ Backlog | Medium | No tenant context at login — prefetch on tenant selection, not login |
...
- **Release 8.3** (Calendar Integration — Tapahtumakalenteri): 9 requirements — 4 ✅ (CAL-1, CAL-2, CAL-3, CAL-4), 5 pending (CAL-5, CAL-6, MP3 gap analysis, MP8 localization, MP9 branding). Migrates PowerShell calendar scripts to Node.js platform. Subsumes R47, supersedes R41. MP3/MP8/MP9 moved from R8.1
