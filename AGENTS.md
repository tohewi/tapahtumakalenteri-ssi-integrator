...
- **Always tag releases:** When a release is completed and merged to `main`, create and push a matching annotated git tag using the requirement release number (for example `v7.2`, `v7.4.1`, `v7.7.1`). Keep tag names aligned with the release number recorded in `docs/requirements/requirements.md` and `docs/RELEASE-NOTES.md`.
